...
namespace Cours6Exercice
{
...

    {
        ...
        private static Dictionary<string, Dictionary<string, List<string>>> ServerTODO = new Dictionary<string, Dictionary<string, List<string>>>();

        //internal static string ConnectionString = "user id = sa; " +
        //                   "password=Blade880;" +
        //                   "server=localhost;" +//174.94.71.98
        ...
        internal static bool SqlNewRead(string command)
        {
            bool Check = true;
            try
            {
                ...
                SqlCommand myCommand = new SqlCommand("select * from table", TODOSqlConn);
        ...
        private static void CreateTodoDB()
        {
            ...
            string CreateDB = $"CREATE TABLE {NewDataBaseName} (id int identity primary key, usernames varchar(30));";
            //SqlConnectOpen(CreateDB);
            ConnectOpen = OpenConnectionToSQLServ();
            if (ConnectOpen)
            {
                CommandSent = SqlNewCommand(CreateDB);
                ...
                if (!CommandSent)
                {
                    Console.WriteLine("Cannot Send Command");
                    CloseConnectionToSQLServ();
                }
            }
            if(!ConnectOpen)
            {
                Console.WriteLine("Cannot connect to SQL DATABASE");
            }

           
               // Console.WriteLine($"DataBase {NewDataBaseName} Could not be created !");
    ...
    {
        StreamWriter SAVE = new StreamWriter("SERVER.txt");

        //Dictionary<string key, Dictionary<string val/key, List<string>values>>();
        //var allo = new ServiceStack.Text.JsonObject.ValueCollection(ServerTODO.Values);
        foreach (var k in ServerTODO)    //1
        {

            int x = 1;
            //      SAVE.Write(string.Format("{0}", ServiceStack.Text.JsonObject.ValueCollection(k.Key, k.Value) ));
            // < string Datab user
            foreach (var i in ServerTODO)       //1
            {
                //   SAVE.Write(string.Format("{0},", i.Value));
                ++x;
                foreach (var u in i.Value.Keys)          //2
                {
                    //         SAVE.Write(string.Format("{0},", u));
                }
                SAVE.WriteLine();
            }
        }
    ...
    static void Main(string[] args)
    {
            ...
            TODOSqlConn.ConnectionString = ConfigurationManager.AppSettings["TODODBCON"];
            TODOMENU();
    }
